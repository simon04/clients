...
export default class AutofillServiceImpl implements AutofillServiceInterface {
  constructor(
  ...
  getFormsWithPasswordFields(pageDetails: AutofillPageDetails): FormData[] {
    ...
    const passwordFields = AutofillServiceImpl.loadPasswordFields(
      pageDetails,
      true,
      true,
      false,
  ...
  private generateFillScript(
    ...
    if (fields && fields.length) {
      ...
      fields.forEach((f) => {
        if (AutofillServiceImpl.hasValue(f.name)) {
      ...
      pageDetails.fields.forEach((field) => {
        ...
        if (matchingIndex > -1) {
          ...
          AutofillServiceImpl.fillByOpid(fillScript, field, val);
        }
      });
    }
  ...
  private generateLoginFillScript(
    ...
    if (!login.password || login.password === "") {
      ...
      fillScript = AutofillServiceImpl.setFillScriptForFocus(filledFields, fillScript);
      return fillScript;
    }

    let passwordFields = AutofillServiceImpl.loadPasswordFields(
      pageDetails,
      false,
      false,
    ...
    if (!passwordFields.length && !options.onlyVisibleFields) {
      ...
      passwordFields = AutofillServiceImpl.loadPasswordFields(
        pageDetails,
        true,
        true,
        options.onlyEmptyFields,
    ...
    if (!passwordFields.length && !options.skipUsernameOnlyFill) {
      ...
      pageDetails.fields.forEach((f) => {
        if (
          ...
          AutofillServiceImpl.fieldIsFuzzyMatch(f, AutoFillConstants.UsernameFieldNames)
        ) {
          usernames.push(f);
        }
      });
    ...
    usernames.forEach((u) => {
      ...
      AutofillServiceImpl.fillByOpid(fillScript, u, login.username);
    });
    ...
    passwords.forEach((p) => {
      ...
      AutofillServiceImpl.fillByOpid(fillScript, p, login.password);
    });
  ...
  private generateCardFillScript(
    ...
    pageDetails.fields.forEach((f) => {
      if (AutofillServiceImpl.forCustomFieldsOnly(f)) {
        return;
      }
      ...
      for (let i = 0; i < CreditCardAutoFillConstants.CardAttributes.length; i++) {
        ...
        if (
          ...
          AutofillServiceImpl.isFieldMatch(
            f[attr],
          ...
          !fillFields.code &&
          AutofillServiceImpl.isFieldMatch(f[attr], CreditCardAutoFillConstants.CVVFieldNames)
        ) {
          fillFields.code = f;
          break;
        } else if (
          !fillFields.brand &&
          AutofillServiceImpl.isFieldMatch(f[attr], CreditCardAutoFillConstants.CardBrandFieldNames)
        ) {
          fillFields.brand = f;
          break;
        }
    ...
    if (fillFields.expMonth && AutofillServiceImpl.hasValue(card.expMonth)) {
      ...
      AutofillServiceImpl.fillByOpid(fillScript, fillFields.expMonth, expMonth);
    }

    if (fillFields.expYear && AutofillServiceImpl.hasValue(card.expYear)) {
      ...
      AutofillServiceImpl.fillByOpid(fillScript, fillFields.expYear, expYear);
    }

    if (
      fillFields.exp &&
      AutofillServiceImpl.hasValue(card.expMonth) &&
      AutofillServiceImpl.hasValue(card.expYear)
    ) {
  ...
  private generateIdentityFillScript(
    ...
    pageDetails.fields.forEach((f) => {
      ...
      for (let i = 0; i < IdentityAutoFillConstants.IdentityAttributes.length; i++) {
        ...
        if (
          ...
          AutofillServiceImpl.isFieldMatch(f[attr], IdentityAutoFillConstants.FirstnameFieldNames)
        ) {
          fillFields.firstName = f;
          break;
        } else if (
          !fillFields.middleName &&
          AutofillServiceImpl.isFieldMatch(f[attr], IdentityAutoFillConstants.MiddlenameFieldNames)
        ) {
          fillFields.middleName = f;
          break;
        } else if (
          !fillFields.lastName &&
          AutofillServiceImpl.isFieldMatch(f[attr], IdentityAutoFillConstants.LastnameFieldNames)
        ) {
          fillFields.lastName = f;
          break;
        } else if (
          !fillFields.title &&
          AutofillServiceImpl.isFieldMatch(f[attr], IdentityAutoFillConstants.TitleFieldNames)
        ) {
          fillFields.title = f;
          break;
        } else if (
          !fillFields.email &&
          AutofillServiceImpl.isFieldMatch(f[attr], IdentityAutoFillConstants.EmailFieldNames)
        ) {
          fillFields.email = f;
          break;
        } else if (
          ...
          !fillFields.address1 &&
          AutofillServiceImpl.isFieldMatch(f[attr], IdentityAutoFillConstants.Address1FieldNames)
        ) {
          fillFields.address1 = f;
          break;
        } else if (
          !fillFields.address2 &&
          AutofillServiceImpl.isFieldMatch(f[attr], IdentityAutoFillConstants.Address2FieldNames)
        ) {
          fillFields.address2 = f;
          break;
        } else if (
          !fillFields.address3 &&
          AutofillServiceImpl.isFieldMatch(f[attr], IdentityAutoFillConstants.Address3FieldNames)
        ) {
          fillFields.address3 = f;
          break;
        } else if (
          !fillFields.postalCode &&
          AutofillServiceImpl.isFieldMatch(f[attr], IdentityAutoFillConstants.PostalCodeFieldNames)
        ) {
          fillFields.postalCode = f;
          break;
        } else if (
          !fillFields.city &&
          AutofillServiceImpl.isFieldMatch(f[attr], IdentityAutoFillConstants.CityFieldNames)
        ) {
          fillFields.city = f;
          break;
        } else if (
          !fillFields.state &&
          AutofillServiceImpl.isFieldMatch(f[attr], IdentityAutoFillConstants.StateFieldNames)
        ) {
          fillFields.state = f;
          break;
        } else if (
          !fillFields.country &&
          AutofillServiceImpl.isFieldMatch(f[attr], IdentityAutoFillConstants.CountryFieldNames)
        ) {
          fillFields.country = f;
          break;
        } else if (
          !fillFields.phone &&
          AutofillServiceImpl.isFieldMatch(f[attr], IdentityAutoFillConstants.PhoneFieldNames)
        ) {
          fillFields.phone = f;
          break;
        } else if (
          !fillFields.username &&
          AutofillServiceImpl.isFieldMatch(f[attr], IdentityAutoFillConstants.UserNameFieldNames)
        ) {
          fillFields.username = f;
          break;
        } else if (
          !fillFields.company &&
          AutofillServiceImpl.isFieldMatch(f[attr], IdentityAutoFillConstants.CompanyFieldNames)
        ) {
    ...
    if (fillFields.name && (identity.firstName || identity.lastName)) {
      let fullName = "";
      if (AutofillServiceImpl.hasValue(identity.firstName)) {
        fullName = identity.firstName;
      }
      if (AutofillServiceImpl.hasValue(identity.middleName)) {
        if (fullName !== "") {
          fullName += " ";
        }
        fullName += identity.middleName;
      }
      if (AutofillServiceImpl.hasValue(identity.lastName)) {
        if (fullName !== "") {
          fullName += " ";
        }
    ...
    if (fillFields.address && AutofillServiceImpl.hasValue(identity.address1)) {
      let address = "";
      if (AutofillServiceImpl.hasValue(identity.address1)) {
        address = identity.address1;
      }
      if (AutofillServiceImpl.hasValue(identity.address2)) {
        if (address !== "") {
          address += ", ";
        }
        address += identity.address2;
      }
      if (AutofillServiceImpl.hasValue(identity.address3)) {
        if (address !== "") {
          address += ", ";
        }
  ...
  private makeScriptActionWithValue(
    ...
    let doFill = false;
    if (AutofillServiceImpl.hasValue(dataValue) && field) {
      if (field.type === "select-one" && field.selectInfo && field.selectInfo.options) {
        for (let i = 0; i < field.selectInfo.options.length; i++) {
          const option = field.selectInfo.options[i];
          for (let j = 0; j < option.length; j++) {
            if (
              AutofillServiceImpl.hasValue(option[j]) &&
    ...
    if (doFill) {
      filledFields[field.opid] = field;
      AutofillServiceImpl.fillByOpid(fillScript, field, dataValue);
    }
  }
  ...
  private fieldPropertyIsMatch(field: any, property: string, name: string): boolean {
    ...
    if (!AutofillServiceImpl.hasValue(fieldVal)) {
      return false;
    }
  ...
  static fieldIsFuzzyMatch(field: AutofillField, names: string[]): boolean {
    if (AutofillServiceImpl.hasValue(field.htmlID) && this.fuzzyMatch(names, field.htmlID)) {
      return true;
    }
    if (AutofillServiceImpl.hasValue(field.htmlName) && this.fuzzyMatch(names, field.htmlName)) {
      return true;
    }
    if (
      AutofillServiceImpl.hasValue(field["label-tag"]) &&
      this.fuzzyMatch(names, field["label-tag"])
    ) {
      return true;
    }
    if (
      AutofillServiceImpl.hasValue(field.placeholder) &&
      this.fuzzyMatch(names, field.placeholder)
    ) {
      return true;
    }
    if (
      AutofillServiceImpl.hasValue(field["label-left"]) &&
      this.fuzzyMatch(names, field["label-left"])
    ) {
      return true;
    }
    if (
      AutofillServiceImpl.hasValue(field["label-top"]) &&
      this.fuzzyMatch(names, field["label-top"])
    ) {
      return true;
    }
    if (
      AutofillServiceImpl.hasValue(field["label-aria"]) &&
